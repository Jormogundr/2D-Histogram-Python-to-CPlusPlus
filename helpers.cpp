...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	vector< vector<float> > newGrid(row, vector<float>(cols, 0)); // Outputs nothing without intializing the 2D vector with 0's
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	vector < vector <float> > newGrid(grid.size(), vector<float> (grid[0].size(), 0)); // initialize a matrix of 0's of size grid.row x grid.col
		...
		for (int j = 0; j < width; j++){
			...
			for (int dx = -1; dx < 2; dx++){
				for (int dy = -1; dy < 2; dy++){
			        ...
			        new_i = modulo(i + dy, height);
                    new_j = modulo(j + dx, width);
...
// Modulo function to handle unique modulo cases
int modulo(int a, int b){ 
	return a >= 0 ? a % b : ( b - abs( a%b )) % b; // condition ? result_if_true : result_if_false
	}
